# TuJe — Chunk 1 Backend — Claude Code Instructions

**Goal:** Add three additive endpoints (`evaluate-answer`, `commit-answer`, `advance-interaction`) that split the fused `submit-answer` into evaluate / commit / advance. **Do not modify** `submit-answer`, `answer_processing_orchestrator.py`, or any service file. Two changes only: (1) create one new file, (2) insert into `routers/session_router.py`.

Work on `~/Desktop/tuje-analyze-api`. No SQL, no schema changes. After applying, do not commit until curl tests in Part 3 pass.

---

## PART 1 — Create the new orchestrator file

Create a new file at `~/Desktop/tuje-analyze-api/answer_split_orchestrator.py` with **exactly** this content:
...
async def advance_after_interaction(interaction_id: str, user_id: str, db_pool: asyncpg.Pool) -> Dict:
    ...
    if not cycle_complete:
        # Idempotency: if the next interaction already exists, return it.
        async with db_pool.acquire() as conn:
    ...
    # Idempotency: if the cycle is already completed, a prior advance ran. Signal
    # already_advanced so a duplicate Continue can't double-increment / re-open.
    async with db_pool.acquire() as conn:
...
## PART 2 — Insert into `routers/session_router.py`

Two insertions. **Do not** change any existing line.

### 2.1 Add request/response models

Find this line (the last field of `SubmitAnswerResponse`, appears once):
```
...
### 2.2 Add the three endpoints

Find this line (start of the hint section, appears once):
```
...
## PART 3 — Verify (no commit until these pass)

Deploy is safe (new endpoints, unused by any client). After Render redeploys, test against one active adaptive interaction.

You need a live `session_interaction_id` whose interaction is `active` (start an adaptive session, grab the first interaction id from logs/DB), and the test user id.

**3.1 Evaluate (voice) — interaction must stay `active`:**
```
curl -s -X POST https://<your-render-host>/api/session/evaluate-answer \
  -H "Content-Type: application/json" \
  -d '{"interaction_id":"<SESSION_INT_ID>","user_id":"<USER_ID>","answer_mode_used":"voice","original_transcript":"bonjour je m appelle remi"}' \
  | python3 -m json.tool
```
Then in TablePlus:
```sql
SELECT status, attempts_count, interaction_score, final_answer_id
FROM session_interaction WHERE id = '<SESSION_INT_ID>';
```
Expect: `status = active`, `attempts_count` incremented, `interaction_score` NULL, `final_answer_id` NULL. Response has `answer_id`, `verdict`, `similarity_score`.

**3.2 Commit — interaction becomes `completed`, no next created:**
```
curl -s -X POST https://<your-render-host>/api/session/commit-answer \
  -H "Content-Type: application/json" \
  -d '{"interaction_id":"<SESSION_INT_ID>","answer_id":"<ANSWER_ID_FROM_3.1>"}' \
  | python3 -m json.tool
```
```sql
SELECT status, interaction_score, final_answer_id, completed_at
FROM session_interaction WHERE id = '<SESSION_INT_ID>';
SELECT interaction_number, status FROM session_interaction
WHERE cycle_id = (SELECT cycle_id FROM session_interaction WHERE id = '<SESSION_INT_ID>')
ORDER BY interaction_number;
```
Expect: this interaction `completed` with a score and `final_answer_id`; **no** interaction_number+1 row yet.

**3.3 Advance — next interaction appears:**
```
curl -s -X POST https://<your-render-host>/api/session/advance-interaction \
  -H "Content-Type: application/json" \
  -d '{"interaction_id":"<SESSION_INT_ID>","user_id":"<USER_ID>"}' \
  | python3 -m json.tool
```
Expect (mid-cycle): `next_interaction_id`, `next_brain_interaction_id`, `interaction_number` populated; `cycle_complete=false`. Verify the new row now exists in the query above.

**3.4 Idempotency:**
- Re-run 3.2 → same recap, no second completion.
- Re-run 3.3 → returns the **same** next interaction (mid-cycle), not a duplicate. Confirm only one interaction_number+1 row exists.

**3.5 Cycle / session ends** (optional, if you can drive a cycle to interaction 7): after committing #7, advance should return `next_cycle` (cycles 1–2) or `session_complete=true` + `session_summary` (after cycle 3). Re-running advance returns `already_advanced=true` with no duplicate cycle.

If all pass: commit. The client switch to these endpoints + the two-panel UI is the next chunk.
