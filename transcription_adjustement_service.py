...
# Use the same pattern as your existing files
            ...
            async with pool.acquire() as conn:
                ...
                for row in rows:
                    ...
                    if row['entity_type_id'] == 'entityNumber':
                        if 'entitynumber' in row['transcription_adjusted'].lower():
                            self.vocab_cache['entitynumber_patterns'].append(vocab_entry)
                        elif re.match(r'^\d+$', row['transcription_fr']):
    ...
    def _handle_un_une_subprocess(self, text: str, pre_adjustment_happened: bool) -> str:
        ...
        for i, word in enumerate(words):
            if word.lower() == 'entitynumber':
                ...
                context_before = ' '.join(words[max(0, i-1):i])
...
async def test_adjustment_cases():
    """Test the complete adjustment process with your examples"""
    test_cases = [
        {
        ...
        {
            "name": "Numbers higher than one, no un/une",
            "input": "Cinq cafés coûtent 14 euros à Paris",
            "expected_pre": "entityNumber cafés coûtent entityNumberentityNumber euros à Paris",
            "expected_final": "entitybeverage coutent entityprice entitycity"
        },
        {
        ...
        {
            "name": "Missing vocabulary case",
            "input": "Je parle français avec un ami tous les matins",
            "expected_pre": "Je parle français avec un ami tous les matins",
            "expected_final": "je parle entitylanguage avec un vocabnotfound tous les matins"
        }
    ]
    ...
    return {"test_results": results}    adjusted_transcript: str # Final result
